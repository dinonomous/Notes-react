...
function App() {
    ...
    useEffect(() => {
        const savedNotes = JSON.parse(localStorage.getItem('Dinesh-notesapplications-3'));
    ...
    function editNote() {
        ...
        return (
            <div className="note-overlay" onClick={() => setEditNoteIndex(null)}>
                <div className="note-overlay-content bg-dark" onClick={(e) => e.stopPropagation()}>
                    <textarea value={input || initialContent} onChange={(e) => setInput(e.target.value)} className='bg-dark border border-3 rounded' />
                    <button onClick={() => {
    ...
    function addNote() {
        return (
            <div className="note-overlay" onClick={() => { setAddNoteState(false) }}>
                <div className="note-overlay-content bg-dark" onClick={(e) => e.stopPropagation()}>
                    <textarea value={input} onChange={(e) => setInput(e.target.value)} className='bg-dark border border-3 rounded' />
                    <button onClick={() => {
                        if (input.trim() !== "") {
    ...
    return (
        <main className="container-fullwidth m-0 p-0 d-flex flex-column">
            <div className="container-fullwidth bg-dark text-light d-flex justify-content-between navme">
                <div className="add my-auto ms-3" onClick={() => setAddNoteState(true)}>
                    <i>
                        <img src={plus} alt="" />
                    </i>
                </div>
